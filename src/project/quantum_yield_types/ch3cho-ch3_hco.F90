...
!   local variables
    ...
    !> read netcdf file quantum yield data
        if( found ) then
          ...
          call die_msg( 600000004, Iam//'expects to read a netCDF data file' )
        endif

        if( size(DataTray) < iONE ) then
          call die_msg( 600000001, Iam//'something wrong with read_netcdf_file for '//trim(netcdfFileSpec%to_char()) )
        endif
        if( size(dataTray(1)%data,dim=2) < 3 ) then
          call die_msg( 600000002, Iam//'dataTray(1) data array has < 3 columns' )
        endif
        ...
        do colNdx = 2,3
          ...
          call ch3cho_ch3_hco_quantum_yield_obj%addpnts( config, data_lambda, data_qyld )
